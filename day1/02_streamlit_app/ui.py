...
def record_audio():
    """
    音声を録音して返す関数（最新streamlit-webrtc対応版）
    """
    st.write("🎤 マイクから録音する")
    ctx = webrtc_streamer(
        key="audio",
...
def display_chat_page(pipe):
    ...
    if audio_bytes:
        st.success("音声が録音されました。文字起こししています...")
        transcribed_text = transcribe_audio(audio_bytes)
        st.success(f"文字起こし結果: {transcribed_text}")
        ...
        with st.spinner("モデルが回答を生成中..."):
            answer, response_time = generate_response(pipe, transcribed_text)
